// DepthRollingGraph.tsx
import { useState, useImperativeHandle, forwardRef, useEffect, useRef } from "react";
import { motion } from "framer-motion";
import Chart from "chart.js/auto";

export type DepthSample = { depth: number; ts?: number };

export type DepthRollingGraphProps = {
  windowSeconds?: number;
  minY?: number;
  maxY?: number;
  color?: string;       // warna garisan depth
  fill?: boolean;       // isi bawah graf
  units?: string;       // default 'm'
  targetDepth?: number; // garisan sasaran (optional)
  reverseYAxis?: boolean; // jika true, axis Y terbalik (lebih dalam = ke bawah)
};

export type DepthRollingGraphRef = {
  push: (sample: DepthSample) => void;
  clear: () => void;
  setPaused: (paused: boolean) => void;
};

const DEFAULT_COLOR = "#38bdf8"; // sky-400

const DepthRollingGraph = forwardRef<DepthRollingGraphRef, DepthRollingGraphProps>(
  (
    {
      windowSeconds = 60,
      minY,
      ...
      units = "m",
      targetDepth,
      reverseYAxis = false, // set ke true kalau nak lebih dalam turun ke bawah
    },
    ref
  ) => {
    ...
    const [lastDepth, setLastDepth] = useState<number | undefined>(undefined);
          ...
          datasets: [
            {
              ...
              backgroundColor: fill ? `${color}33` : undefined, // ~20% opacity
            },
            ...(typeof targetDepth === "number"
              ? [
                  {
                    label: "Target",
                    data: [
                      { x: Date.now() - windowSeconds * 1000, y: targetDepth },
                      { x: Date.now(), y: targetDepth },
                    ],
    ...
    const applyTrimAndUpdate = (ts: number) => {
      ...
      // Auto-scale Y jika min/max tak diberi
      ...
      // Sync garisan target
      if (typeof targetDepth === "number" && chart.data.datasets.length > 1) {
        const ds = chart.data.datasets[1];
        (ds.data as any) = [
          { x: cutoff, y: targetDepth },
          { x: ts, y: targetDepth },
        ];
      }
    ...
    const push = (sample: DepthSample) => {
      if (paused) return;
      const ts = sample.ts ?? Date.now();
      dataRef.current.push({ x: ts, y: sample.depth });
      setLastDepth(sample.depth);
      applyTrimAndUpdate(ts);
    };

    const clear = () => {
      dataRef.current = [];
      setLastDepth(undefined);
    ...
    return (
      <div className="w-[400px] h-full bg-[#21262E] text-zinc-100 border-r border-slate-700">
        <div className="p-2">
          ...
          >
            <ValuePill label="Depth" value={lastDepth} units={units} color={color} />
          </motion.div>
        </div>
      </div>
    );
...
export default DepthRollingGraph;

...
const createOverlay$1 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  createOverlay
...
function createSplashWindow(timeoutMs) {
  ...
  const publicRoot = process.env.VITE_PUBLIC || path$m.join(process.env.APP_ROOT || process.cwd(), "public");
...
function getRendererDist() {
  return process.env.APP_ROOT ? path$m.join(process.env.APP_ROOT, "dist") : path$m.join(__dirname, "..", "..", "dist");
}
function getDevUrl() {
...
function createMainWindow() {
  ...
  if (devUrl) {
    ...
    win2.loadFile(path$m.join(getRendererDist(), "index.html"));
  }
  return win2;
}
function createOverlayEditorWindow() {
  ...
  if (devUrl) {
    ...
    win2.loadFile(path$m.join(getRendererDist(), "index.html"), { query: { window: "overlay-editor" } });
  }
  return win2;
}
...
function openObs() {
  ...
  if (!fs$j.existsSync(OBS_WORKING_DIR)) {
...
async function exitOBS() {
  ...
  const timeoutMs = 3e3;
  const requestData = {
    reason: `requested by ${path$m.basename(process.execPath)}`,
...
function resolveServerPath() {
  const appRoot = process.env.APP_ROOT || path$m.join(__dirname, "..", "..");
  const asarPath = path$m.join(appRoot, "drawing-service", "server.js");
...
async function startBrowserSourceService(port) {
  try {
    ...
    try {
      overlayImagesDir = path$m.join(app.getPath("userData"), "overlay-images");
    } catch {
      overlayImagesDir = "";
    }
...
ipcMain.handle("db:getAllTasks", async (_event, projectId) => {
  try {
    ...
    const plain = items.map((t2) => {
      var _a, _b, _c, _d;
      return {
        _id: ((_b = (_a = t2._id) == null ? void 0 : _a.toString) == null ? void 0 : _b.call(_a)) ?? t2._id,
        projectId: ((_d = (_c = t2.projectId) == null ? void 0 : _c.toString) == null ? void 0 : _d.call(_c)) ?? t2.projectId,
        name: t2.name,
        remarks: t2.remarks,
...
async function editDive(diveId, updates) {
  ...
  if (typeof updates.name === "string") set2.name = updates.name.trim();
  if (typeof updates.remarks === "string") set2.remarks = updates.remarks.trim();
  if (typeof updates.started === "boolean") set2.started = updates.started;
...
function extractNodePathNames(root2) {
  const names = [];
  let cur = root2;
...
async function getExportedProjectHierarchy(projectId) {
  ...
  for (const s of sessions) {
    ...
    const entries = [s.preview, s.ch1, s.ch2, s.ch3, s.ch4];
...
ipcMain.handle("db:getExportedProjectHierarchy", async (_e, projectId) => {
  try {
...
async function editProject(projectId, updates) {
  ...
  const set2 = { updatedAt: now };
  if (typeof updates.name === "string") set2.name = updates.name.trim();
  if (typeof updates.client === "string") set2.client = updates.client.trim();
  if (typeof updates.contractor === "string") set2.contractor = updates.contractor.trim();
  if (typeof updates.vessel === "string") set2.vessel = updates.vessel.trim();
  if (typeof updates.location === "string") set2.location = updates.location.trim();
  if (updates.hasOwnProperty("lastSelectedDiveId")) {
    const v = updates.lastSelectedDiveId;
    set2.lastSelectedDiveId = typeof v === "string" ? v.trim() || null : null;
  }
  if (updates.hasOwnProperty("lastSelectedTaskId")) {
    const v2 = updates.lastSelectedTaskId;
    set2.lastSelectedTaskId = typeof v2 === "string" ? v2.trim() || null : null;
  }
  if (updates.hasOwnProperty("lastSelectedNodeId")) {
    const v3 = updates.lastSelectedNodeId;
    set2.lastSelectedNodeId = typeof v3 === "string" ? v3.trim() || null : null;
  }
  if (updates.hasOwnProperty("lastSelectedOverlayCh1Id")) {
    const v4 = updates.lastSelectedOverlayCh1Id;
    set2.lastSelectedOverlayCh1Id = typeof v4 === "string" ? v4.trim() || null : null;
  }
  if (updates.hasOwnProperty("lastSelectedOverlayCh2Id")) {
    const v5 = updates.lastSelectedOverlayCh2Id;
    set2.lastSelectedOverlayCh2Id = typeof v5 === "string" ? v5.trim() || null : null;
  }
  if (updates.hasOwnProperty("lastSelectedOverlayCh3Id")) {
    const v6 = updates.lastSelectedOverlayCh3Id;
    set2.lastSelectedOverlayCh3Id = typeof v6 === "string" ? v6.trim() || null : null;
  }
  if (updates.hasOwnProperty("lastSelectedOverlayCh4Id")) {
    const v7 = updates.lastSelectedOverlayCh4Id;
    set2.lastSelectedOverlayCh4Id = typeof v7 === "string" ? v7.trim() || null : null;
  }
...
ipcMain.handle("app:setActiveSessionId", async (_event, id) => {
  try {
...
const createOverlayComponent$1 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
...
async function handleUpload(input) {
  ...
  if (input.sourcePath) {
    ...
    if (!stat2.isFile()) throw new Error("Source is not a file");
    const filename = buildTargetFilename(path$m.basename(src2));
    const dest = path$m.join(imagesDir, filename);
    fs$j.copyFileSync(src2, dest);
  ...
  if (input.bytesBase64) {
    ...
    fs$j.writeFileSync(dest, buffer);
    const fileUrl = `file://${dest.replace(/\\/g, "/")}`;
    const httpUrl = buildHttpUrl(filename);
    return { absolutePath: dest, fileUrl, httpUrl, filename };
  }
...
async function addProjectLog(input) {
  ...
  const mm = pad2$2(now.getMonth() + 1);
  const dd = pad2$2(now.getDate());
  const hh = pad2$2(now.getHours());
  const mi = pad2$2(now.getMinutes());
  const ss = pad2$2(now.getSeconds());
    ...
    for (const name of files) {
      try {
        ...
        entries.push({
          ...
          filename: name,
          size: stat2.size,
          modifiedAt: new Date(stat2.mtimeMs).toISOString()
        });
      } catch {
      }
    }
...
function updateRecordingState(patch2) {
  recordingState = { ...recordingState, ...patch2 };
}
...
ipcMain.handle("recording:updateState", async (_e, patch2) => {
  try {
    updateRecordingState(patch2 || {});
    try {
      for (const win2 of BrowserWindow.getAllWindows()) {
        try {
          win2.webContents.send("recordingStateChanged");
        } catch {
        }
      }
    } catch {
...
async function takeSnapshots(payload) {
  ...
  const doOne = async (idx, hint) => {
    ...
    const filePath = path$m.join(outDir || process.cwd(), `${baseName}.png`);
    try {
    ...
    try {
      ...
      if (sessionId && Array.isArray(files) && files.length) {
        ...
        if (Object.keys(snapshots).length) {
          try {
            await editSession(sessionId, snapshots);
          } catch {
          }
        }
      }
...
async function listSceneChildren(sceneName) {
  try {
    const obs = getObsClient();
    if (!obs) return [];
...
async function setActiveInputForScene(sceneName, sourceIndex, inputType) {
  ...
  const enableId = async (id, on) => {
    if (id == null) return;
    try {
      await obs.call("SetSceneItemEnabled", { sceneName: containerName, sceneItemId: id, sceneItemEnabled: on });
    } catch {
    }
  };
    ...
    for (let index = 1; index <= 4; index++) {
      ...
      if (deviceId) {
        try {
          ...
          const items = Array.isArray(prop == null ? void 0 : prop.propertyItems) ? prop.propertyItems : [];
          const match = items.find((it) => String((it == null ? void 0 : it.itemValue) ?? (it == null ? void 0 : it.value) ?? "") === deviceId);
...
async function setCaptureDeviceInput(sceneName, sourceIndex, deviceId) {
  ...
  try {
    const res = await obs.call("GetSceneItemList", { sceneName });
    const children = Array.isArray(res == null ? void 0 : res.sceneItems) ? res.sceneItems : [];
    vcd = children.find((it) => isVcdName(String((it == null ? void 0 : it.sourceName) ?? ""), sourceIndex));
  } catch {
    vcd = void 0;
  }
  if (!vcd) return false;
...
async function getActiveInputsForScene(sceneName) {
  ...
  const result = createDefaultState();
  if (!obs) return result;
  const enabledOf = async (containerName, it) => {
    if (typeof (it == null ? void 0 : it.sceneItemEnabled) === "boolean") return !!it.sceneItemEnabled;
    try {
      const en = await obs.call("GetSceneItemEnabled", { sceneName: containerName, sceneItemId: Number(it == null ? void 0 : it.sceneItemId) });
      return !!(en == null ? void 0 : en.sceneItemEnabled);
    } catch {
      return false;
    }
...
function ensureDirectoryFor(fileOrDir) {
  const ext = path$m.extname(fileOrDir).toLowerCase();
  if (ext === ".json") {
    const dir2 = path$m.dirname(fileOrDir);
    try {
      fs$j.mkdirSync(dir2, { recursive: true });
    } catch {
    }
...
ipcMain.handle("db:exportOverlay", async (_event, input) => {
  try {
    ...
    const overlays = db.collection("overlays");
    const components = db.collection("overlay_components");
    const selectedId = (input == null ? void 0 : input.overlayId) && String(input.overlayId).trim() ? String(input.overlayId).trim() : getSelectedOverlayLayerId() || "";
    if (!selectedId) throw new Error("No overlay selected");
    const overlay = await overlays.findOne({ _id: new ObjectId(selectedId) });
    if (!overlay) throw new Error("Overlay not found");
    const list = await components.find({ overlayId: new ObjectId(selectedId) }).sort({ createdAt: 1 }).toArray();
    const exportOverlay = {
    ...
    const json2 = JSON.stringify({ overlay: exportOverlay, components: exportComponents }, null, 2);
    const target = ensureDirectoryFor(destPath);
    let filePath;
    if (target.isFile) {
      const baseDir = path$m.dirname(target.filePath);
      const base = sanitizeName(exportOverlay.name);
      filePath = path$m.join(baseDir, `${base}.json`);
    } else {
      const base = sanitizeName(exportOverlay.name);
      filePath = path$m.join(target.filePath, `${base}.json`);
    }
    fs$j.writeFileSync(filePath, json2, "utf8");
...
async function openSerialDevice(portName, baudRate, stopBits, dataBits, parity) {
  ...
  const handleLine = (line) => {
    try {
      emitter.emit("data", line.replace(/\r$/, ""));
    } catch {
    }
  };
  ...
  return {
    ...
    close: async () => {
      try {
        parser.off("data", handleLine);
      } catch {
      }
      port.off("error", handleError2);
...
function getSerialDeviceState() {
  return serialDeviceState;
}
function updateSerialDeviceState(patch2) {
  serialDeviceState = { ...serialDeviceState, ...patch2 };
}
...
ipcMain.handle("serial:updateDeviceState", async (_e, patch2) => {
  try {
    updateSerialDeviceState(patch2 || {});
    return { ok: true };
  } catch (err) {
    const message = err instanceof Error ? err.message : "Unknown error";
    return { ok: false, error: message };
  ...
  try {
    ...
    const onData = (chunk) => {
      try {
        ...
        if (serialDeviceState.data.length > 1e3) serialDeviceState.data.splice(0, serialDeviceState.data.length - 1e3);
        const parts = line.split(",").map((s) => s.trim());
        const prev = serialDeviceState.currentFields;
        serialDeviceState.currentFields = parts.map((v, i) => {
          var _a;
...
async function closeCurrentSerial() {
  if (!live) return true;
  try {
    await live.close();
  } catch {
...
ipcMain.handle("serial:setFieldKey", async (_e, index, key) => {
  try {
...
async function getCOMPorts() {
  try {
    ...
    const comPaths = ports.map((p) => p.path).filter((p) => Boolean(p)).filter((p) => /^COM\d+$/i.test(p));
    ...
    console.error("Failed to list COM ports:", error2);
    return [];
  }
}
function isJsonFile(p) {
  return path$m.extname(p).toLowerCase() === ".json";
}
async function createOverlaySafe(name) {
  var _a, _b, _c, _d;
  let base = name.trim() || "Imported Overlay";
  let attempt = 0;
  while (true) {
    const candidate = attempt === 0 ? base : `${base} (${attempt + 1})`;
    await ((_a = global.ipcMain) == null ? void 0 : _a.handle);
    try {
      const out2 = await ((_b = global.electronIpcInvoke) == null ? void 0 : _b.call(global, "db:createOverlay", { name: candidate }));
      if ((out2 == null ? void 0 : out2.ok) && out2.data) return String(out2.data);
    } catch {
    }
    try {
      const { createOverlay: createOverlay2 } = await Promise.resolve().then(() => createOverlay$1);
      const created = await createOverlay2({ name: candidate });
      return String(((_d = (_c = created == null ? void 0 : created._id) == null ? void 0 : _c.toString) == null ? void 0 : _d.call(_c)) || created);
    } catch (err) {
...
ipcMain.handle("db:importOverlay", async (_event, input) => {
  var _a, _b;
  try {
    const sourcePath = String((input == null ? void 0 : input.sourcePath) || "").trim();
    if (!sourcePath || !isJsonFile(sourcePath)) throw new Error("sourcePath must be a .json file");
    const raw = fs$j.readFileSync(sourcePath, "utf8");
    const parsed = JSON.parse(raw);
    const overlay = parsed == null ? void 0 : parsed.overlay;
    const items = Array.isArray(parsed == null ? void 0 : parsed.components) ? parsed.components : [];
    const nameRaw = String((input == null ? void 0 : input.nameOverride) || ((overlay == null ? void 0 : overlay.name) || "Imported Overlay"));
    let overlayId;
    try {
      const { createOverlay: createOverlay2 } = await Promise.resolve().then(() => createOverlay$1);
      const created = await createOverlay2({ name: nameRaw });
      overlayId = String(((_b = (_a = created == null ? void 0 : created._id) == null ? void 0 : _a.toString) == null ? void 0 : _b.call(_a)) || created);
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      if (/already exists/i.test(message)) {
    ...
    const { createOverlayComponent: createOverlayComponent2 } = await Promise.resolve().then(() => createOverlayComponent$1);
    for (const c of items) {
      try {
        await createOverlayComponent2({
          overlayId,
          type: c.type,
          name: void 0,
          // let the DB assign default names (type-#)
          x: Number(c.x) || 0,
          y: Number(c.y) || 0,
          ...
          textStyle: typeof c.textStyle === "object" ? c.textStyle : void 0,
          customText: typeof c.customText === "string" ? c.customText : void 0,
          dateFormat: typeof c.dateFormat === "string" ? c.dateFormat : void 0,
          twentyFourHour: typeof c.twentyFourHour === "boolean" ? c.twentyFourHour : void 0,
          useUTC: typeof c.useUTC === "boolean" ? c.useUTC : void 0,
          dataType: typeof c.dataType === "string" ? c.dataType : void 0,
          nodeLevel: typeof c.nodeLevel === "number" ? c.nodeLevel : void 0,
          imagePath: typeof c.imagePath === "string" ? c.imagePath : void 0,
          // likely blank from export
          opacity: typeof c.opacity === "number" ? c.opacity : void 0,
          projectDetail: typeof c.projectDetail === "string" ? c.projectDetail : void 0
        });
      } catch {
      }
    }
...
ipcMain.handle("db:createDataKey", async (_event, name) => {
  try {
...
async function fetchDataKeys() {
  ...
  return docs.map((d) => {
    var _a, _b;
    return {
      _id: ((_b = (_a = d._id) == null ? void 0 : _a.toString) == null ? void 0 : _b.call(_a)) ?? String(d._id),
      name: d.name,
      createdAt: d.createdAt,
      updatedAt: d.updatedAt
    };
...
function formatHMS$1(ms2) {
  const totalSeconds = Math.floor(ms2 / 1e3);
  const hh = pad2$1(Math.floor(totalSeconds / 3600));
  const mm = pad2$1(Math.floor(totalSeconds % 3600 / 60));
  const ss = pad2$1(totalSeconds % 60);
  return `${hh}:${mm}:${ss}`;
}
function coerceTime$1(val) {
  if (typeof val === "number" && Number.isFinite(val)) return formatHMS$1(val);
...
ipcMain.handle("db:addEventLog", async (_event, input) => {
  try {
    const created = await addEventLog(input);
    const id = created == null ? void 0 : created._id;
    const plainId = typeof (id == null ? void 0 : id.toHexString) === "function" ? id.toHexString() : String(id);
...
async function editEventLog(id, patch2) {
  ...
  const $set = { updatedAt: /* @__PURE__ */ new Date() };
  if (typeof patch2.eventName !== "undefined" && patch2.eventName !== null) $set.eventName = String(patch2.eventName).trim();
  if (typeof patch2.eventCode !== "undefined" && patch2.eventCode !== null) $set.eventCode = String(patch2.eventCode).trim();
  const st = coerceTime(patch2.startTime);
  if (typeof st !== "undefined") $set.startTime = st;
  const et = coerceTime(patch2.endTime);
  if (typeof et !== "undefined") $set.endTime = et;
  const res = await col.updateOne({ _id }, { $set });
  return { matched: res.matchedCount, modified: res.modifiedCount };
}
ipcMain.handle("db:editEventLog", async (_event, id, patch2) => {
  try {
    const r = await editEventLog(id, patch2 || {});
...
async function createWindow() {
  ...
  try {
    ...
    if (online) {
      ...
      main$1.autoUpdater.on("update-available", async () => {
        try {
          const choice = await (win == null ? void 0 : win.webContents.executeJavaScript('new Promise(r=>{const ok=confirm("An update is available. Download and install now?"); r(ok)})'));
          if (choice) main$1.autoUpdater.downloadUpdate();
        } catch {
        }
      });
      main$1.autoUpdater.on("update-downloaded", async () => {
        try {
          const choice = await (win == null ? void 0 : win.webContents.executeJavaScript('new Promise(r=>{const ok=confirm("Update downloaded. Install and restart now?"); r(ok)})'));
          if (choice) main$1.autoUpdater.quitAndInstall();
        } catch {
        }
      });
      try {
...
ipcMain.handle("data-config-window:minimize", async () => {
  try {
    dataConfigWin == null ? void 0 : dataConfigWin.minimize();
    return true;
  } catch {
    return false;
  }
...
ipcMain.handle("data-config-window:close", async () => {
  try {
    dataConfigWin == null ? void 0 : dataConfigWin.close();
    return true;
  } catch {
    return false;
  }
...
ipcMain.handle("dialog:selectDirectory", async () => {
  ...
  try {
    ...
    if (res.canceled || !((_a = res.filePaths) == null ? void 0 : _a[0])) return { ok: false, error: "cancelled" };
